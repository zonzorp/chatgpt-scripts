...
if [ $? -ne 0 ]; then
    echo "Failed to execute local script for loghost" >&2
    exit 1
fi

./configure-host.sh -hostentry webhost 192.168.16.4 $VERBOSE
if [ $? -ne 0 ]; then
    echo "Failed to execute local script for webhost" >&2
    exit 1
fi
